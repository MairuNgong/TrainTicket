    ...
    def create_widgets_meal(self) :
        ...
        for meal in meal_list:
                for menu,price in meal.items():
                    ...
                    meal_count = ttk.Spinbox(Meal_frame, from_=1, to=10, command=self.calculate_total_price)
                    meal_count.set(0)
                    meal_count.pack(side=RIGHT,anchor=E)
                    
                    self.all_spinbox.append({"meal": meal_count, "price": price})
       

        # submit_button = ttk.Button(list_frame,text='Submit',command=self.calculate_total_price)
        # submit_button.pack()


        #meal_payload = {"pagename":"first_summation","meals":meal_total_price}
                    
class First_Summation_Page(ttk.Frame) :
    def __init__(self, frame_controller) :
        super().__init__(frame_controller)
    ...
    def create_widgets_first_sum(self) :
        for widget in self.winfo_children(): widget.destroy()
...
class Seat_page(ttk.Frame) :
    ...
    def send_payload(self, req_label) :
        if self.validate_input(req_label) :
            payload_choose_seat = {"pagename": "summation", "body": self.seat_payload_list}
    ...
    def create_widgets_seat(self) :
        ...
        warning_label = ttk.Label(self, bootstyle="danger")
        
        confirm_buttun = ttk.Button(self, bootstyle="seccess", text="Confirm", command=lambda : self.send_payload(warning_label))
        confirm_buttun.pack(pady=30)

        payload_back_button = {"pagename": "bogie","body": {"train_id":self.controller.cache["train_id"], "departure_time": self.controller.cache["departure_time"], "destination_time": self.controller.cache["destination_time"]}}
    ...
    def create_user_info_frame(self) :
        ...
        payload_ticket_button = {"pagename":"ticket"}
        ...
        payload_logout_button = {"pagename":"main_from_logout"}
...
class Bogie_Page(ttk.Frame) :
    ...
    def create_widgets_bogie(self) :
        ...
        for data in bogie_list :
            ...
            if self.controller.cache["user"] != None: payload_choose_bogie_button = {"pagename": "seat", "body": {"train_id": data[key][sub_key_list[0]], "bogie_id": key}}
            else : payload_choose_bogie_button = {"pagename": "login_from_bogie"}
        ...
        payload_back_button = {"pagename": "train","body": {"combo_departure":self.controller.cache["departure_station"], "combo_destination": self.controller.cache["destination_station"], "my_date": self.controller.cache["departure_date"], "entry_amount": self.controller.cache["amount"]}}
        back_button = ttk.Button(self, bootstyle="info", text="go back", command=lambda: self.controller.set_page_state(payload_back_button))
        back_button.pack(side=tk.BOTTOM, anchor=tk.W, padx=10, pady=40)
...
class Train_Page(ttk.Frame) :
    ...
    def create_widgets_train(self) :
        ...
        for data in route_list :
            ...
            payload_choose_train_button = {"pagename": "bogie", "body": {"train_id": key, "departure_time": data[key][sub_key_list[2]], "destination_time": data[key][sub_key_list[3]]}}
        ...
        payload_back_button = {"pagename": "route"}
        back_button = ttk.Button(self, bootstyle="info", text="go back", command=lambda: self.controller.set_page_state(payload_back_button))
        back_button.pack(side=tk.BOTTOM, anchor=tk.W, padx=10, pady=40)
...
class Route_Page(ttk.Frame) :
    ...
    def create_widgets_route(self):
        ...
        payload_submit_button = {"pagename": "train", "body": {"combo_departure":combo_departure, "combo_destination": combo_destination, "my_date": my_date, "entry_amount": entry_amount,"error":error_label}}
        ...
        payload_back_button = {"pagename": "main"}
        back_button = ttk.Button(self, bootstyle="info", text="go back", command=lambda: self.controller.set_page_state(payload_back_button))
        back_button.pack(side=tk.BOTTOM, anchor=tk.W, padx=10, pady=40)
...
class Register_Page(ttk.Frame) :
    ...
    def create_widgets_register(self) :
        ...
        if self.controller.cache["register from login"]:
            payload_back_button = {"pagename": "login_from_register"}
        else:
            payload_back_button = {"pagename":"main"}
        back_button = ttk.Button(self, bootstyle="info", text="go back", command=lambda: self.controller.set_page_state(payload_back_button))
        back_button.pack(side=tk.BOTTOM, anchor=tk.W, padx=10, pady=40)
...
class Login_Page(ttk.Frame) :
    ...
    def create_widgets_login(self) :
        ...
        if self.controller.cache["login from bogie"]:
            ...
            payload_login_button = {"pagename": "bogie_from_login", "body": {"username" : username_entry, "password" : password_entry,"error":error_label}}
            payload_back_button = {"pagename": "bogie","body": {"train_id": self.controller.cache["train_id"], "departure_time": self.controller.cache["departure_time"], "destination_time": self.controller.cache["departure_time"]}}
        else:
            error_label = ttk.Label(form_label, text="", foreground="red",bootstyle="inverse-light")
            payload_login_button = {"pagename": "main_from_login_reg", "body": {"username" : username_entry, "password" : password_entry,"error":error_label}}
            payload_back_button = {"pagename": "main"}
        ...
        payload_register_button = {"pagename": "register_from_login"}
...
class Main_Page(ttk.Frame) :
    ...
    def create_widgets_main(self) :
        ...
        payload_reservation_button = {"pagename": "route"}
        ...
        payload_login_button = {"pagename": "login"}
        ...
        payload_register_button = {"pagename": "register"}
...
class FrameController(ttk.Frame) :
    ...
    def set_page_state(self, payload):
        if payload["pagename"] == "login":
            self.__frames["login"] = Login_Page(self)

        elif payload["pagename"] == "login_from_bogie":
            self.cache["login from bogie"] = True
            self.__frames["login"] = Login_Page(self)
            payload["pagename"] = "login"

        elif payload["pagename"] == "login_from_register":
            self.cache["register from login"] = False
            self.__frames["login"].destroy()
            self.__frames["login"] = Login_Page(self)
            payload["pagename"] = "login"
        
        elif payload["pagename"] == "bogie_from_login":
            ...
            payload["pagename"] = "bogie"

        elif payload["pagename"] == "register":
            self.__frames["register"] = Register_Page(self)

        elif payload["pagename"] == "register_from_login":
            self.cache["register from login"] = True
            self.__frames["register"] = Register_Page(self)
            payload["pagename"] = "register"
        elif payload["pagename"] == "ticket":
        ...
        elif payload["pagename"] == "bogie" :
            self.__result_bogie_list = self.submit_req_train(payload["body"])
            self.__frames["bogie"] = Bogie_Page(self)
        
        elif payload["pagename"] == "main_from_login_reg" :
            response = self.validate("login",payload["body"])
            if response != "pass":
                return
            response = self.submit_login(payload["body"])
            ...
            payload["pagename"] = "main"
        
        elif payload["pagename"] == "main_from_logout":
            self.submit_logout()
            self.__frames["main"].show_login_and_register_button()
            payload["pagename"] = "main"
            
        elif payload["pagename"] == "seat" :
            self.__result_seat_list = self.submit_req_bogie(payload["body"])
            self.__frames["seat"] = Seat_page(self)
        
        
        elif payload["pagename"] == "summation" :
            self.submit_req_seat(payload["body"])
    ...
    def submit_req_seat(self, body) :
        ...
        #! เปลี่ยน backend รับเป็นstr หมด
        ...
        self.cache["meals"] = response["meals"]
